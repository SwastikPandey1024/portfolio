...
export const SalesPulsePage: React.FC = () => {
  const project = FLAGSHIP_PROJECTS.find((p) => p.id === 'salespulse');
  ...
  return (
    <Section spacing="default">
      <PageContainer size="wide">
        <Link
          to="/#work"
          className="inline-flex items-center gap-1.5 font-mono text-xs text-content-muted hover:text-semantic-emerald transition-colors mb-8"
        >
        ...
        <div className="flex flex-col gap-6 max-w-4xl">
          <div className="flex items-center gap-3">
            <Badge variant="emerald">{project.numericId} // {project.domain}</Badge>
          ...
          <p className="font-display text-2xl text-semantic-emerald font-semibold">
            ...
            {project.liveUrl && (
              <a href={project.liveUrl} target="_blank" rel="noopener noreferrer">
                <Button variant="primary" className="bg-emerald-600 hover:bg-emerald-500 border-emerald-500/50 shadow-glow-emerald">
                  Live Streamlit App <ArrowUpRight className="w-4 h-4 ml-1" />
                </Button>
              </a>
            )}
            <a href={project.repositoryUrl} target="_blank" rel="noopener noreferrer">
              <Button variant="outline">
                <Github className="w-4 h-4 mr-1.5" /> View GitHub Repository
              </Button>
            </a>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 my-12">
          <div className="surface-card p-6 border-l-4 border-l-semantic-emerald">
          ...
          <div className="surface-card p-6 border-l-4 border-l-semantic-emerald">
            <h3 className="font-display font-bold text-lg text-content-primary mb-2">
              Why Machine Learning & Ensembles?
            </h3>
            <p className="font-body text-sm text-content-muted leading-relaxed">
              {project.whyAi}
            </p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 my-12">
          {project.metrics.map((m) => (
            <Metric key={m.label} value={m.value} label={m.label} context={m.context} highlightColor="emerald" />
          ))}
        </div>
        ...
        <div className="my-12">
          ...
          <p className="font-body text-sm text-content-muted mb-4">
            End-to-end data pipeline from raw retail transactional data to multi-horizon predictive decision support.
          </p>
        ...
        <div className="my-12 rounded-xl overflow-hidden border border-border bg-surface">
          ...
          <div className="p-4 bg-surface-raised border-t border-border font-mono text-xs text-content-muted">
            SalesPulse AI enterprise analytics dashboard overview showcasing Superstore dataset KPIs and forecast horizons.
          </div>
        </div>
